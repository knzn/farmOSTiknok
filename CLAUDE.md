...
└── (app)/                     # Protected routes
    ...
    System routes:
    └── oauthredirect.tsx      # Google OAuth redirect handler (calls maybeCompleteAuthSession)
```
...
GET   /api/auth/me
POST  /api/auth/google           Google OAuth (mobile + web)
...
- EAS env vars (preview): `EXPO_PUBLIC_API_URL=https://api.tiknok.app/api`, `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID=...`
- `expo-build-properties` plugin: `android.usesCleartextTraffic: false` (production HTTPS)
...
- APK auto-uploaded to DO Spaces after each build: `tiknok-media/app/tiknok.apk`

### Google OAuth (Android)
- Android OAuth client: `593043566086-2gp9li3b9tmnrnn6qsgrpr6km63u0dto` (package: `com.tiknok.app`)
- Web OAuth client: `593043566086-bnst6v6jkdssq1eccehbclklms68kn5e` (authorized redirect: `https://auth.expo.io/@tiknok/tiknok`, `https://tiknok.app`)
- **Custom URI Scheme must be ENABLED** on the Android OAuth client in Google Cloud Console → Credentials → Android client → Advanced settings
- Redirect URI used: `tiknok://oauthredirect` (auto-generated by expo-auth-session)
- Intent filter for `com.tiknok.app` scheme registered in app.json (catches `com.tiknok.app:/oauthredirect`)
- `app/oauthredirect.tsx` route calls `WebBrowser.maybeCompleteAuthSession()` to complete the flow
...
# Mobile — apps/mobile/.env (local dev)
EXPO_PUBLIC_API_URL=http://192.168.137.1:3001/api
EXPO_PUBLIC_WS_URL=ws://192.168.137.1:3001
EXPO_PUBLIC_WEB_URL=https://tiknok.app
EXPO_PUBLIC_GOOGLE_CLIENT_ID=593043566086-2gp9li3b9tmnrnn6qsgrpr6km63u0dto.apps.googleusercontent.com
EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID=593043566086-bnst6v6jkdssq1eccehbclklms68kn5e.apps.googleusercontent.com

# Production EAS env vars (set via eas env:create, override .env at build time)
# EXPO_PUBLIC_API_URL=https://api.tiknok.app/api
# EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID=593043566086-bnst6v6jkdssq1eccehbclklms68kn5e.apps.googleusercontent.com
```
...
```
Droplet IP:   104.248.154.42
Domain:       tiknok.app
API:          https://api.tiknok.app  (PM2: api, port 3001)
Web:          https://tiknok.app      (PM2: web, port 3000)
Repo path:    /var/www/tiknok
Deploy:       git pull → pnpm install → pnpm --filter @app/types build → pnpm --filter api build → pm2 restart all
```
...
- **API** — `https://api.tiknok.app` (DigitalOcean droplet, Nginx + SSL, PM2)
- **Web app** — `https://tiknok.app` (Next.js, same droplet, PM2)
- **APK** — `https://tiknok-media.sgp1.cdn.digitaloceanspaces.com/app/tiknok.apk` (DO Spaces)
- **Domain** — `tiknok.app` (Cloudflare DNS, SSL via Certbot)
...
- [x] Google OAuth — mobile (expo-auth-session) + web (NextAuth)
- [x] Auth mobile screens (login, register) — Google + email/password
...
**Landing page (tiknok.app)**
- [x] Hero section with Android APK download button
- [x] Direct APK download from DO Spaces (permanent URL)
- [x] Safari PWA install instructions for iPhone users
...
- ~~Facebook OAuth~~ (removed — not needed, Google + email is sufficient)
