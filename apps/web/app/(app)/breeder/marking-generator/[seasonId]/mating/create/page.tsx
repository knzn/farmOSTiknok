...
export default function CreateMatingPage() {
  const params   = useParams()
  const router   = useRouter()
  const seasonId = params.seasonId as string

  const [maleName, setMaleName]           = useState('')
  const [henNames, setHenNames]           = useState([''])
  ...
  return (
    <div className="p-6 max-w-xl">
      ...
      <h1 className="text-ink text-2xl font-bold mt-1 mb-6">New Mating</h1>
      ...
      <div className="mb-5">
        ...
        <div className="flex items-center bg-card border border-rim rounded-xl w-36">
          <button
            onClick={() => adjustCount(henNames.length - 1)}
            disabled={henNames.length <= 1}
            className="w-12 h-12 flex items-center justify-center text-2xl font-light text-ink disabled:text-rim"
          >−</button>
